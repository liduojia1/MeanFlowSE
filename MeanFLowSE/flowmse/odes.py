...
class FLOWMATCHING(ODE):
    ...
    # mu_t = (1-t)*x_clean + t*y, sigma_t = (1-t)*sigma_min + t*sigma_max,  t ∈ (0,1]
    @staticmethod
    ...
    def _mean(self, x1, t, y):
        return (1 - t)[:, None, None, None] * x1 + t[:, None, None, None] * y

    def _std(self, t):
        return (1 - t) * self.sigma_min + t * self.sigma_max

    def marginal_prob(self, x1, t, y):
        return self._mean(x1, t, y), self._std(t)

    def prior_sampling(self, shape, y):
        if shape != y.shape:
            warnings.warn(f"Target shape {shape} does not match shape of y {y.shape}! Ignoring target shape.")
        T = getattr(self, "T_rev", 1.0)
        std = self._std(torch.ones((y.shape[0],), device=y.device) * T)
        z = torch.randn_like(y)
        x_T = y + z * std[:, None, None, None]
        return x_T, z

    def der_mean(self, x1, t, y):
        return y - x1

    def der_std(self, t):
        return torch.full_like(t, self.sigma_max - self.sigma_min)
